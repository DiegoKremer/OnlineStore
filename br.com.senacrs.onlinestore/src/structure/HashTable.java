...
public class HashTable {
	...
	public void insert(Book value) {
		...
		if (table[firstHash] == null) {
			...
			/* If inserted to prevent infinite loop causing stackoverflow - NEEDS REVIEW */
			if (cycleControl < 4) {
				insert(tempOldValue);
				usedCapacity++;
				checkLoadFactor();
	...
	/* Converts the long value received from ISBN field
	 * to an Integer by removing the first 4 digits of
	 * the code, which has a high repeat rate */
	...
	/* Increase the size of the hash table when it reaches its
